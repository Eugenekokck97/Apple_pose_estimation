...
class AESimpleHead(DeconvHead):
    ...
    def get_loss(self, outputs, targets, masks, joints):
        ...
        heatmaps_losses, push_losses, pull_losses, circleloss = self.loss(
        ...
        for idx in range(len(targets)):
            ...
            if pull_losses[idx] is not None:
                ...
                if 'pull_loss' not in losses:
                    ...
                    losses['pull_loss'] += pull_loss
            if circleloss[idx] is not None:
                circleloss = circleloss[idx].mean(dim=0)
                if 'circleloss' not in losses:
                    losses['circleloss'] = circleloss
                else:
                    losses['circleloss'] += circleloss
